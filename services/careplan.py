...
async def create_careplan(db, resident_id: str, careplan_data: CarePlanCreate):
    ...
    if "created_date" in careplan_dict and isinstance(careplan_dict["created_date"], datetime.date):
        careplan_dict["created_date"] = datetime.datetime.combine(careplan_dict["created_date"], datetime.time.min)
...
async def update_careplan(db, resident_id: str, careplan_id: str, update_data: CarePlanCreate):
    if not ObjectId.is_valid(careplan_id):
        raise HTTPException(status_code=400, detail="Invalid careplan ID")

    update_dict = update_data.dict(exclude_unset=True)
    if "created_date" in update_dict and isinstance(update_dict["created_date"], datetime.date):
        update_dict["created_date"] = datetime.datetime.combine(update_dict["created_date"], datetime.time.min)
