...
async def list_reports(request: Request, status: Optional[str] = None, db=Depends(get_db)):
...
@limiter.limit("10/minute")
async def edit_report(request: Request, report_id: str, report: ReportCreate, db=Depends(get_db)):
